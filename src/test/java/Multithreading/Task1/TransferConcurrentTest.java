...
import Multithreading.Task1.UsingConcurrent.TransferConcurrent;
...
public class TransferConcurrentTest {
    ...
    public void moneyTransferBetweenDifferentAccounts() throws Exception {
        Thread thread = new Thread(new TransferConcurrent(source, destination, amount));
    ...
    public void throwExceptionIfOneOfAccountsIsNull() throws Exception {
        Thread thread = new Thread(new TransferConcurrent(source, null, amount));
        thread.start();
    }

    @Test(expected = IllegalArgumentException.class)
    public void throwExceptionIfAmountIsNegative() throws Exception {
        Thread thread = new Thread(new TransferConcurrent(source, destination, -500));
        thread.start();
    }

    @Test(expected = IllegalArgumentException.class)
    public void throwExceptionIfSourceDoesntHaveEnoughMoney() {
        Thread thread = new Thread(new TransferConcurrent(source, destination, sourceBalance + 100));
        thread.start();
    }
}
